...
2. Chép `Code.gs`, `ManagerDashboard.gs`, `Index.html` và `appsscript.json` từ thư mục này.
3. Không chạy hàm migration, không đổi Spreadsheet ID production và không xóa các tab lịch sử.
4. Trong Google Cloud OAuth client, bảo đảm Authorized JavaScript origin có `https://taqhung95-cloud.github.io`; broker đăng nhập tiếp tục dùng `docs/data-entry-login.html`.
