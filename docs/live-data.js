(() => {
  ...
  function curveChart(series,axisLabels,maxValue,unit){const W=760,H=250,L=43,R=45,T=14,B=38,pw=W-L-R,ph=H-T-B,max=Math.max(1,maxValue),x=i=>L+(axisLabels.length===1?pw/2:i*pw/(axisLabels.length-1)),y=v=>T+ph-Math.max(0,Math.min(max,v))/max*ph;const grid=[0,.25,.5,.75,1].map(t=>{const yy=y(t*max);return '<line x1="'+L+'" y1="'+yy+'" x2="'+(W-R)+'" y2="'+yy+'"></line><text x="'+(L-7)+'" y="'+(yy+4)+'" text-anchor="end">'+Math.round(t*max)+'</text>'}).join(''),axes=axisLabels.map((label,i)=>'<text class="sg-curve-axis" x="'+x(i)+'" y="'+(H-10)+'" text-anchor="middle">'+label+'</text>').join(''),curves=series.map((s,si)=>{const pts=s.values.map((v,i)=>[x(i),y(v)]),color=chartColors[si%chartColors.length],dots=pts.map((p,i)=>'<circle cx="'+p[0]+'" cy="'+p[1]+'" r="3.2" style="fill:'+color+'"><title>'+s.name+' · '+axisLabels[i]+': '+Math.round(s.values[i])+unit+'</title></circle>').join('');return '<path class="sg-curve-line" d="'+curvePath(pts)+'" style="stroke:'+color+'"></path>'+dots}).join(''),legend=series.map((s,i)=>'<span><i style="background:'+chartColors[i%chartColors.length]+'"></i>'+s.name.replace(' Service Center',' SC').replace(' Center','')+'</span>').join('');return '<div class="sg-curve-chart"><svg viewBox="0 0 '+W+' '+H+'" preserveAspectRatio="none" role="img"><g class="sg-curve-grid">'+grid+'</g>'+axes+curves+'</svg><div class="sg-curve-legend">'+legend+'</div></div>'}
  function renderCenters() {
    ...
    const profiles=cs.map(function(x){const flow=x.outflowInflowRatio===null||x.outflowInflowRatio===undefined?0:Math.min(100,x.outflowInflowRatio*100),control=Math.max(0,100-(x.overdueRate||0)*100),sla=x.slaRate===null||x.slaRate===undefined?(x.medianDays===null?0:Math.max(0,100-Math.max(0,x.medianDays-7)*12.5)):x.slaRate*100;return {name:x.center,values:[flow,control,sla,x.coverage||0]};});
    q('#sg-center-month-table').innerHTML = curveChart(profiles,['Ra/vào','Không quá hạn','SLA 7 ngày','Dữ liệu đủ'],100,'%')+'<div class="sg-chart-definition">Điểm chuẩn hóa dùng để so sánh hình dạng vận hành; số tuyệt đối nằm trên thẻ center.</div>';
